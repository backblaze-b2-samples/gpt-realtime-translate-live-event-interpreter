...
`<EventCard />` (in `components/events/event-card.tsx`) is the default
primitive for the live-interpretation app. It renders any `Event` with:

- A two-line linked title and a monospace event id
- A status badge (`scheduled` / `live` / `ended`)
- Source-language plus target-language chips (renders BCP-47 codes in
  small monospace badges; replaces the parent starter's bitrate strip)
- A metadata strip: `created date · duration`
- Open / Listen / Delete actions; Delete is gated by an `AlertDialog` and
  cascades the entire `events/<id>/` prefix
- An attendee-peak counter (`Users` icon + integer)

### Language chip primitive
The language chip is a thin wrapper over `<Badge variant="outline">` with
`font-mono text-[10px]`. Use it anywhere you need to surface a BCP-47 code
without typography fighting the surrounding text.

### When to use
Default for any list of live-interpretation events stored under the
`events/` prefix. Compose into a grid (`sm:grid-cols-2 xl:grid-cols-3`)
like `events-view.tsx` does, or drop a single card in a detail view.

### When NOT to use
- The full-bucket explorer at `/files` uses the tree-view file browser —
  the card is purpose-built for events and doesn't render arbitrary objects.

### Composing your own events tab
1. Add a query hook to `lib/queries.ts` (mirror `useEvents`).
2. Make sure your records serialize to the `Event` shape from
   `packages/shared/src/types.ts`.
3. Reuse `<EventCard />` and `<EventsView />` verbatim — both are net-new
   (non-shadcn) primitives and safe to depend on directly.

The live showcase lives at `/design` under "Event Card".
